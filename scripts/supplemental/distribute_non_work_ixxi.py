...
def calc_fric_fac(cost_skim, dist_skim, _coeff_df, zone_lookup, state):
    ...
    for index, row in _coeff_df.iterrows():
        ...
        LOW_PNR_INDEX = zone_lookup[state.emme_settings.LOW_PNR]
...
def emme_matrix_to_np(trip_purp_list, dictZoneLookup, state):
    ...
    for purpose in trip_purp_list:
        ...
        START_ZONE_INDEX = dictZoneLookup[state.emme_settings.HIGH_TAZ]+1
...
def main(state):
    ...
    ixxi_trips = emme_matrix_to_np(trip_purpose_list, dictZoneLookup, state)
