...
def split_tod_internal(total_trips_by_mode, tod_factors_df):
    ...
    for tod in tod_factors_df.time_of_day.unique():
        ...
        ixxi_work_store = h5py.File(
            "outputs/supplemental/external_work_" + tod + ".h5", "r"
        )
        ...
        for mode, tod_type in tod_dict.items():
            if mode in ["sov", "hov2", "hov3"]:
                tod_dict[mode] = np.array(total_trips_by_mode[mode]) * tod_df[
                    tod_df["mode"] == tod_type
                ]["value"].values[0] + np.array(ixxi_work_store[mode])
            else:
                tod_dict[mode] = (
                    np.array(total_trips_by_mode[mode])
                    * tod_df[tod_df["mode"] == "transit"]["value"].values[0]
                )
