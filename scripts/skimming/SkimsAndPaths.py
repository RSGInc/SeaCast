...
def class_specific_volumes(my_project):
    ...
    for x in range(0, len(mod_skim["classes"])):
        ...
        mod_skim["classes"][x]["results"]["turn_volumes"] = (
            "@p" + my_user_classes["Highway"][x]["Name"]
        )
    skim_traffic(mod_skim)
...
def hdf5_trips_to_Emme(my_project, hdf_filename):
    ...
    # Add airport trips
    airport_filename = os.path.join(state.emme_settings.airport_output_dir, 'airport_demand_' + my_project.tod+'.omx')
    skims_logger.debug('Adding airport trips from airport_demand_{}.omx'.format(my_project.tod))
    ...
    omx_file.close()
    skims_logger.debug('Finished adding airport trips from airport_demand_{}.omx'.format(my_project.tod))
