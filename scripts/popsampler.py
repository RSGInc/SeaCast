...
def sample_hhs(group):
    ...
    if len(sample)==0:
        ...
        if group.sample_rate.min()%1 == 0:
            sample = pd.concat([group for i in range(0,int(group.sample_rate.min()))])
            sample['hhexpfac'] = 1.0 / group.sample_rate.min()
        else:
...
def runPopSampler(tazSampleRateFileName, popsynFileName, popsynOutFileName):
    ...
    tazSampleRateFileName = os.path.join( tazSampleRateFileName)
    ...
    incbins = [households['hhincome'].min()-1, 50000, 100000, households['hhincome'].max()+1]
