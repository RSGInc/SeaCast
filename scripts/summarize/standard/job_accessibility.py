...
def get_transit_information(bank):
    ...
    transit_time_df = pd.melt(
        transit_time_df,
        id_vars="from",
        value_vars=list(transit_time_df.columns[0:zonesDim]),
        var_name="to",
...
def bike_walk_jobs_access(links, nodes, parcel_df, parcel_geog, distances, geo_list):
    ...
    links = links[
        links["to_node_id"].isin(nodes.index) & links["from_node_id"].isin(nodes.index)
    ]
...
def main(state):
    ...
    auto_time_df = pd.melt(
        auto_time_df,
        id_vars="from",
        value_vars=list(auto_time_df.columns[0:zonesDim]),
        var_name="to",
        value_name="travel_time",
    )
