...
def main(state):
    ...
    seatac_routes = pd.read_csv(r'inputs\model\daysim\lookup\seatac_transit_routes.csv')
    ...
    # Write SeaTac transit routes
    seatac_airport_routes = pd.read_csv(r'inputs\model\daysim\lookup\seatac_airport_transit_routes.csv')
    ...
    df_daily = (
        ...
        .agg({"@tveh": "sum", "@facilitytype": "first", '@subarea_flag':'first'})
        .reset_index()
    )
    ...
    df_daily["county"] = df_daily["countyid"].astype(int).astype('str').map(state.summary_settings.county_map)
    df_daily.to_csv(
    ...
    df_county_facility_counts = (
        df_daily.groupby(["county", "@facilitytype",'subarea_flag'])[["observed", "modeled"]]
        .sum()
        .reset_index()
    )
    ...
    df["county"] = df["@countyid"].astype(int).astype(str).map(state.summary_settings.county_map)
    df.to_csv(os.path.join(validation_output_dir, "hourly_volume.csv"), index=False)

    # Truck counts
    truck_counts = pd.read_sql(
        "SELECT * FROM daily_counts WHERE year=" + str(state.input_settings.base_year),
        con=state.conn,
    )
    ...
    df_truck_daily['diff_medt'] = df_truck_daily['modeled_medt']-df_truck_daily['mtrk_vehicles']
    df_truck_daily['perc_diff_medt'] = df_truck_daily['diff_medt']/df_truck_daily['mtrk_vehicles']
    df_truck_daily[['modeled_medt','mtrk_vehicles']] = df_truck_daily[['modeled_medt','mtrk_vehicles']].apply(lambda x: round(x,2))
    df_truck_daily['diff_hvyt'] = df_truck_daily['modeled_hvyt']-df_truck_daily['htrk_vehicles']
    df_truck_daily['perc_diff_hvyt'] = df_truck_daily['diff_hvyt']/df_truck_daily['htrk_vehicles']
    df_truck_daily[['modeled_hvyt','htrk_vehicles']] = df_truck_daily[['modeled_hvyt','htrk_vehicles']].apply(lambda x: round(x,2))
    df_truck_daily['modeled'] = df_truck_daily['modeled_medt']+df_truck_daily['modeled_hvyt']
    df_truck_daily['observed'] = df_truck_daily['mtrk_vehicles']+df_truck_daily['htrk_vehicles']
    ...
    _df = (
        df_model[["@tveh", "screenline_id", "@subarea_flag"]]
        .groupby(["screenline_id", "@subarea_flag"])
        .sum()[["@tveh"]]
        .reset_index()
    )
    ...
    df_speed = (
        df_speed.groupby(["Corridor_Number", "tod", '@subarea_flag'])
