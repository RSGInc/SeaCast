...
def write_nb(sheet_name, nb_path, output_path):
    ...
    with open(nb_path + r"/" + sheet_name + ".ipynb") as f:
        ...
        if sys.version_info > (3, 0):
            ...
            text = (
                "jupyter nbconvert --to HTML " + nb_path + r"//" + sheet_name + ".ipynb"
            )
            os.system(text)
        ...
        os.rename(
            (os.path.join(nb_path, sheet_name + ext)),
            os.path.join(os.getcwd(), output_path, sheet_name + ext),
        )
    # except:
...
def main():
    ...
    for sheet_name in [
        ...
        print(sheet_name)
        write_nb(
            sheet_name, "scripts/summarize/notebooks/validation", r"outputs/validation/"
        )
