...
def summarize_network(state, df, writer):
    ...
    for metric in ["VMT", "VHT", "delay"]:
        _df = pd.pivot_table(
            df,
            values=metric,
            index=['tod','period', '@subarea_flag', '@concurrency', '@truck_route'],
            columns="facility_type",
            aggfunc="sum",
        ).reset_index()
        _df = sort_df(df=_df, sort_list=state.network_settings.tods, sort_column="tod")
...
#                                project.delete_matrix(matrix)
#                                project.delete_extra_attribute("@eline")
...
#                        transit_path_analysis = project.m.tool(
...
#                        )
        ...
        # Export turn-level results for multiple attributes
        network = project.current_scenario.get_network()
        _network_df = export_network_turn_attributes(network)
        _network_df['tod'] = project.tod
        network_turn_df = pd.concat([network_turn_df, _network_df])

    output_dict = {
        "outputs/network/network_results.csv": network_df,
        "outputs/network/network_turn_results.csv": network_turn_df,
