...
def main(state):
    ...
    balanced_prod_att = pd.read_csv("outputs/supplemental/7_balance_trip_ends.csv", index_col='taz')
    ...
    # get time of day:
    df_tod = pd.read_sql(
        f"SELECT time_period,truck_type FROM truck_time_of_day_factors WHERE model=='{state.input_settings.abm_model}'",
        con=state.conn)
    
    #truck_matrix_list = pd.concat([truck_matrix_list,
    #                              pd.DataFrame({
    #                                  'matrix_type':['full' for tod in df_tod.time_period],
    #                                  'matrix_name':(df_tod.time_period + '_' + df_tod.truck_type + "_trips").values,
    #                                  'description':(df_tod.time_period + '_' + df_tod.truck_type + "_trips").values,
    #                                  })
    #],
    #axis=0,
    #ignore_index=True
