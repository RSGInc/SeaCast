...
from scripts.network import network_importer, update_employee_shuttle_ttf
...
def daysim_assignment(iteration):
    ...
    ########################################
    if state.input_settings.run_airport_model:
...
def daysim_popsampler(statepe):
    option = state.input_settings.sampling_option
    #read zone district cross file
    zone_district = pd.read_csv(os.path.join(state.input_settings.zone_district_file), usecols = ['taz','geog_name'] )
    zone_district.columns = ['zone_id','district']
    zone_district['sample_rate'] = 0 #initialize

    #get districts for sampling population
    districts = state.input_settings.pop_sample_district.keys()
    ...
    for district, district_sample_name in district_name_map.items():
        zone_district.loc[zone_district['district'] == district, 'sample_rate'] = state.input_settings.pop_sample_district[district_sample_name][option-1] #option-1, as index starts from 0

    #output a text file for popsampler to use
    zone_district[['zone_id','sample_rate']].to_csv(os.path.join(state.input_settings.taz_sample_rate_file), index = False, sep = '\t')
    ...
    with open(abs_config_template_path, 'r') as abs_config:
        for line in abs_config:
            #print line
            settings = line.split('=')
    ...
    #run popsampler
    popsyn_in_file = state.input_settings.households_persons_file
    popsyn_out_file = os.path.join(r'inputs\model\daysim\roster','hh_and_persons_sampled.h5')
    returncode = subprocess.call([sys.executable,'scripts/popsampler.py',state.input_settings.taz_sample_rate_file, popsyn_in_file, popsyn_out_file])
...
def run_all_summaries():
    ...
    subprocess.run(
        "conda activate st_summary2 && python scripts/summarize/validation_network/create_network_validation.py",
        shell=True,
    )
    #subprocess.run(
    #    "conda activate st_summary2 && python scripts/summarize/RTP_summary/create_RTP_summary.py",
    #    shell=True,
    #)
    subprocess.run(
        "conda activate st_summary2 && python scripts/summarize/validation/create_validation_pages.py && conda deactivate",
        shell=True,
    )
...
def run_airport(iteration):
    ...
    if iteration == 0:
        ...
        call_string = 'conda activate seatac_airport && python {} -c {} -e {} -n {}'.format(
            ...
            state.input_settings.num_enplanements,
            state.input_settings.connecting,
        )
...
def main():
    ...
    if state.input_settings.run_import_networks:
        ...
        logger.info("Start adjusting travel time function for employee shuttle")
        update_employee_shuttle_ttf.adjust_ttf(state)
        logger.info("End adjusting travel time function for employee shuttle")
        time_adjust = datetime.datetime.now()
    ...
    if (
        ...
        if state.input_settings.run_daysim_popsampler:
            daysim_popsampler(state.input_settings)
...
if __name__ == "__main__":
    # Set up logging
    logger = logcontroller.setup_custom_logger("main_logger", r"seacast_log.txt")
